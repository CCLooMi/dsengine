...
//		Path p=Paths.get(System.getProperty("user.dir"),"src/test/resources/d.txt");
//		byte[]bs=Files.readAllBytes(p);
//		String s=new String(bs);
//		String[]docs=s.replaceAll("\n", "").split(",");
//		for(int i=0;i<docs.length;i++) {
//			Map<String,Object>doc=new HashMap<>();
//			if("水调歌头".equals(docs[i])) {
		...
		while(true) {
			...
			if(!"exit".equalsIgnoreCase(queryStr)){
				if(!"".equals(queryStr)){
					...
					ResultBean result=engine.search("{info:'水调歌头'}");
					System.out.println(result);
				}
			}else{
				break;
